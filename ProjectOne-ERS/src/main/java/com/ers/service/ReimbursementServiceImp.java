...
import com.ers.dao.ReimbursementDAOImp;
...
public class ReimbursementServiceImp implements ReimbursementService{
	...
	public HashMap<Integer,Reimbursement> getUserTickets(int id) {
		List<Reimbursement> temp = myDao.getUserTickets(id); 
		//store usertickets under the id given to a local list
		HashMap<Integer,Reimbursement> storeReimbursement = new HashMap<Integer, Reimbursement>(); 
		//for each reimbursement stored in the local list store it in a key value pair with the id 
	...
	public HashMap<Integer,Reimbursement> getAllTickets() {
		List<Reimbursement> temp = myDao.getAllTickets(); 
		//local list that will store reimbursements
		HashMap<Integer,Reimbursement> storeReimbursement = new HashMap<Integer, Reimbursement>();
		if(temp == null) { //check if array list is null
			loggy.info("Local reimbursement list is null");
			return null;
			...
			loggy.info("Local reimbursement list has : " + temp.size() + " tickets");
			for(Reimbursement r:temp) {//for each reimbursement, it stores the element in temp 
				//put the <id, reimbursement into the stroeReimbursement HashMap
				storeReimbursement.put(r.getReimId(), r);
			}
